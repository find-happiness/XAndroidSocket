...
public class XTcpClient extends BaseXSocket {
  ...
  private Disposable disposableConnectionThread;
  private Disposable disposableSendThread;
  private Disposable disposableReceiveThread;
  ...
  protected synchronized void onErrorDisConnect(String msg, Throwable e) {
    if (isDisconnected()) {
      return;
    }
    disconnect(msg, e);
  ...
  protected synchronized void disconnect(String msg, Throwable e) {
    if (isDisconnected()) {
      return;
    }
  ...
  private void disposeConnectionThread() {
    if (disposableConnectionThread != null && disposableConnectionThread.isDisposed()) {
      disposableConnectionThread.dispose();
      disposableConnectionThread = null;
    }
  }
  ...
  private void connection() {
    ...
    disposableConnectionThread = Flowable.create((FlowableOnSubscribe<String>) emitter -> {
      try {
        ...
        emitter.onNext("创建连接成功,target=" + mTargetInfo + ",localport=" + localPort);
        ...
        .subscribe(success -> {
          XSocketLog.d(TAG, success);
          notifyConnected();
          onConnectSuccess();
        }, error -> onErrorDisConnect("创建连接失败", error));
  }
    ...
    @Override public void run() {
      try {
        ...
        while (isConnected() && !Thread.interrupted()) {
          ...
          byte[][] decodebytes =
              mTcpConnConfig.getDecodeHelper().execute(result, mTargetInfo, mTcpConnConfig);
  ...
  private void disposeReadThread() {

    if (disposableReceiveThread != null && !disposableReceiveThread.isDisposed()) {
      disposableReceiveThread.dispose();
      disposableReceiveThread = null;
    }
  }
  ...
  private void startReadThread() {
    ...
    disposableReceiveThread = Flowable.create(new FlowableOnSubscribe<TcpMsg>() {
      @Override public void subscribe(FlowableEmitter<TcpMsg> emitter) throws Exception {
        try {
          InputStream is = getSocket().getInputStream();
          while (isConnected()
              && disposableReceiveThread != null
              && !disposableReceiveThread.isDisposed()) {
            byte[] result = mTcpConnConfig.getStickPackageHelper().execute(is);//粘包处理
            if (result == null) {//报错
              emitter.tryOnError(new Exception("tcp Receive 粘包处理失败 " + Arrays.toString(result)));
              break;
            }
            XSocketLog.d(TAG, "tcp Receive 解决粘包之后的数据 " + Arrays.toString(result));
            TcpMsg tcpMsg = new TcpMsg(result, mTargetInfo, TcpMsg.MsgType.Receive);
            ...
            byte[][] decodebytes =
                mTcpConnConfig.getDecodeHelper().execute(result, mTargetInfo, mTcpConnConfig);
            tcpMsg.setEndDecodeData(decodebytes);
            emitter.onNext(tcpMsg);
          }
        } catch (Exception e) {
          XSocketLog.d(TAG, "接受消息错误  " + e);
  ...
  protected void startSendThread() {
    ...
    disposableSendThread = Flowable.create(new FlowableOnSubscribe<TcpMsg>() {
      @Override public void subscribe(FlowableEmitter<TcpMsg> emitter) throws Exception {
        TcpMsg msg;
        try {
          while (isConnected() && (msg = getMsgQueue().take()) != null && (disposableSendThread
              != null && !disposableSendThread.isDisposed())) {
            XSocketLog.d(TAG, "tcp sending msg=" + msg);
            byte[] data = msg.getSourceDataBytes();
            if (data == null) {//根据编码转换消息
              data = CharsetUtil.stringToData(msg.getSourceDataString(),
        ...
        .subscribe(tcpMsg -> {
          ...
          onErrorDisConnect("发送消息失败", error);
        });
  }

  public void disposeSendThread() {

    if (disposableSendThread != null && !disposableSendThread.isDisposed()) {
      disposableSendThread.dispose();
      disposableSendThread = null;
    }
  }
  ...
  private void notifyDisconnected(final String msg, final Throwable e) {
    TcpClientListener l;
    for (TcpClientListener wl : mTcpClientListeners) {
      final TcpClientListener finalL = wl;
      runOnUiThread(new Runnable() {
